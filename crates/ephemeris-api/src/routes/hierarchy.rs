...
pub async fn get_full_hierarchy<E: EventRepository, A: AggregationRepository>(
...
pub async fn get_children<E: EventRepository, A: AggregationRepository>(
    State(state): State<Arc<AppState<E, A>>>,
    Path(epc): Path<String>,
...
pub async fn get_ancestors<E: EventRepository, A: AggregationRepository>(
    State(state): State<Arc<AppState<E, A>>>,
    Path(epc): Path<String>,
) -> Result<Json<Vec<Epc>>, (StatusCode, Json<Value>)> {
    let epc = Epc::new(epc);
